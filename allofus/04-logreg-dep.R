...
logreg_ethnicity <- function (data, subset, outVar) {
  ...
  estimates <- data.frame(
    ...
    model = "minimal",
    n = rep(nobs(fit1)),
    OR = as.numeric(exp(coef(summary(fit1))[2:7])),
    ci.min = as.numeric(exp(confint.default(fit1))[2:7,1]),
    ci.max = as.numeric(exp(confint.default(fit1))[2:7,2]),
    p.value = as.numeric(coef(summary(fit1))[2:7,4]))
...
dep_ethnicity <- read.csv("03-results/logreg_dep_ethnicity.csv")
dep_immigration <- read.csv("03-results/logreg_dep_immigration.csv")
